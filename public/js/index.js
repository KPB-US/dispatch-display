 ...
 */
  ...
  function initMap(el, call) {
    ...
    g.geocode(gr, function(results, status) {
      // console.log('geocoded gr', gr, results);
      if (status == 'OK') {
        if (results.length > 0) {
          ...
          directionsService.route(request, function(response, status) {
            if (status == 'OK') {
              ...
              // see if the found destination is actually somewhere out of the actual
              // service area
              if (!route || route.legs[0].distance.value > 160934) {
                $(callEl).find('.map').empty();
                $(callEl).find('.route').html('route not found within service area');
                console.log('route not found within service area');
              } else {
                if (route.legs.length > 0 && route.legs[0].steps.length > 0) {
                  ...
                  // console.log(route.legs[0].steps.map((e) => e.instructions));
