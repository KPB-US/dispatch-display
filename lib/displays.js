...
function handleConnection(socket) {
  ...
  if (station == null) {
    console.log(CONNECTION_INVALID, socket.conn.remoteAddress);
  ...
  socket.on('calls-log-query', function() {
    console.log('A display at station', station.id, 'requested calls log', socket.conn.remoteAddress);
  ...
  socket.on('disconnect', function(reason) {
     console.log('disconnected', socket.conn.remoteAddress, reason);
...
function handleIncomingData(req, res) {
  ...
  if (station == null) {
    ...
    res.status(200).send('Not configured to handle calls for that station.');
    return;
  }
